...
this_mfile = dbstack('-completenames'); %this seems to work only if the 'save and run' button is pressed
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% INPUT INITIAL TRIM CONDITIONS

% % 30 degree spiral
% omegaB0 = [-0.0009, 0.5520, 0.2948];
% vB0     = [27.7981, 0.0520, 0.3857];
% Delta0  = [0, 0, 0];
% R0      = [ 0.9645,     0.1347,     -0.2271;
%             -0.2640,    0.4816,     -0.8357;
%             -0.0032,    0.8660,     0.5001];
% g0      = [R0, Delta0'; zeros(1,3) 1];
% xihatB0 = [hat(omegaB0), vB0'; zeros(1,4)];
% eulr0   = R2eulr(g0(1:3,1:3));

% %Input control surface deflections in degrees
% deltae_deg = -2.8;          % positive pitch down
% deltaa_deg = 6.8991e-005;   % positive roll right
% deltar_deg = 0;             % positive yaw left

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Maximum range and endurance: STARTING FROM SAME INITIAL CONDITIONS

pitchdes    = 1; %desired pitch angle (degrees)
omegaB0     = [0, 0, 0];
vB0         = [5, 0, 0];
Delta0      = [0, 0, 0];
R0          = eye(3,3);
...
%Input control surface deflections in degrees
deltae_deg = -2.718;    % positive pitch down
deltaa_deg = 0;         % positive roll right
deltar_deg = 0;         % positive yaw left
...
% 1. Strict bus modeling: To enable this diagnostic, set the
%  'Mux blocks used to create bus signals' option to 'error' in
%  Configuration Parameters Dialog -> Diagnostics ->
%  Connectivity.
%  2. Underspecified initialization detection: To enable this
%  diagnostic, set the 'Underspecified initialization
%  detection' to 'Simplified' in Configuration Parameters
%  Dialog -> Diagnostics -> Data Validity -> Model
%  Initialization

% U0=[-0.0006, -0.0000, -0.5279, 0.00003495, 0.00000239, 0.00000266]; %for circle descent
U0 = [0.0085, 0, -0.2639, 0, 0.5173e-7, 0]; % for straight descent
X0 = [8.2248, 0, 0.4043, 0, 0, 0, 0, 0.0202, 0, 0, 0, 0];
[A, B, ~, ~] = linmod('nonlinear_balsa_v2', X0, U0);
% [A,B,~,~]=linmod('nonlinear_balsa_v3',X0,[-2.8, 0, 0]);
...
for i = 1:n_int-1
    ...
    [g, xihatB] = eom_integrator_v1(geo, xihatB, tauB, fB, gbar, g, dt);
